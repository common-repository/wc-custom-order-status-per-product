...
if ( ! class_exists( 'ocwcp_kit' ) ) {
	class ocwcp_kit {
		...
		function admin_menu() {
			add_submenu_page( 'wc-custom-order-status', esc_html__( 'Essential Kit', 'ocwcp' ), esc_html__( 'Essential Kit', 'ocwcp' ), 'manage_options', 'ocwcp_kit', array(
				$this,
				'settings_page'
			) );
		}
		...
		function settings_page() {
            ...
            <div class="wrap">
                ...
                <div class="wp-list-table widefat plugin-install-network">
					...
					$updated      = false;
					$plugins_info = get_site_transient( 'ocwcp_kit_plugins_info' );
											...
											<?php if ( $this->is_plugin_installed( $_plugin ) ) {
												if ( $this->is_plugin_active( $_plugin ) ) {
                                                    ...
                                                    <a href="<?php echo esc_url( $this->deactivate_plugin_link( $_plugin ) ); ?>"
                                                       class="button deactivate-now">
														<?php esc_html_e( 'Deactivate', 'ocwcp' ); ?>
                                                    </a>
													<?php
												} else {
													?>
                                                ...
                                                <a href="<?php echo esc_url( $this->install_plugin_link( $_plugin ) ); ?>"
                                                   class="install-now button wpckit-install-now">
													<?php esc_html_e( 'Install Now', 'ocwcp' ); ?>
                                                </a>
											<?php } ?>
                                        </li>
                                        <li>
                                            <a href="<?php echo esc_url( $details_link ); ?>"
                                               ...
                                               aria-label="<?php echo esc_attr( sprintf( esc_html__( 'More information about %s', 'ocwcp' ), $plugin_info['name'] ) ); ?>"
                                               data-title="<?php echo esc_attr( $plugin_info['name'] ); ?>">
												<?php esc_html_e( 'More Details', 'ocwcp' ); ?>
                                            </a>
                                        </li>
                                    </ul>
                                </div>
							...
							<?php 
								...
								if ( isset( $plugin_info['version'] ) ) { ?>
                                    <div class="column-updated">
										<?php echo esc_html__( 'Version', 'ocwcp' ) . ' ' . $plugin_info['version']; ?>
                                    </div>
								<?php }

								if ( isset( $plugin_info['active_installs'] ) ) { ?>
                                    <div class="column-downloaded">
										<?php echo number_format_i18n( $plugin_info['active_installs'] ) . esc_html__( '+ Active Installations', 'ocwcp' ); ?>
                                    </div>
								<?php }

								if ( isset( $plugin_info['last_updated'] ) ) { ?>
                                    <div class="column-compatibility">
                                        <strong><?php esc_html_e( 'Last Updated:', 'ocwcp' ); ?></strong>
                                        <span><?php printf( esc_html__( '%s ago', 'ocwcp' ), esc_html( human_time_diff( strtotime( $plugin_info['last_updated'] ) ) ) ); ?></span>
                                    </div>
								<?php }
					...
					if ( $updated ) {
						set_site_transient( 'ocwcp_kit_plugins_info', $plugins_info, 24 * HOUR_IN_SECONDS );
					}
					?>
                </div>
            </div>
		...
		public function activate_plugin_link( $plugin) {
			...
			return wp_nonce_url( admin_url( 'admin.php?page=ocwcp_kit&action=activate&plugin=' . $plugin['slug'] . '/' . $plugin['file'] . '#' . $plugin['slug'] ), 'activate-plugin_' . $plugin['slug'] . '/' . $plugin['file'] );
			
		}

		public function deactivate_plugin_link( $plugin ) {
			return wp_nonce_url( admin_url( 'admin.php?page=ocwcp_kit&action=deactivate&plugin=' . $plugin['slug'] . '/' . $plugin['file'] . '#' . $plugin['slug'] ), 'deactivate-plugin_' . $plugin['slug'] . '/' . $plugin['file'] );
		}
	}
